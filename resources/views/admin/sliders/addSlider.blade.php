    ...
    <main>
        <div class="container-fluid px-4">
            ...
            @if(session()->has('error'))
              ...
              @endif
            <div class="row">
                 <form method="post" action="{{ route('sliders.insert') }}" enctype="multipart/form-data">
                  @csrf
                <div class="card-body">
                  <div class="form-group">
                    <label for="short_title">Short Title</label>
                    <input type="text" name="short_title" class="form-control @error('short_title') is-invalid @enderror" value="{{ old('short_title') }}" required autocomplete="short_title" id="short_title">
                    @error('short_title')
                    ...
                    <input type="text" name="long_title" class="form-control @error('long_title') is-invalid @enderror" value="{{ old('long_title') }}" required autocomplete="long_title" id="long_title">
                        @error('long_title')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
